...
-(void)initWithPreparation4KeyboardWithVC:(UIViewController *)targetVC andLiftTheViews:(NSArray *)views2Lift
{
    ...
    class_addMethod([targetVC class], @selector(ZKKeyboardpan:), class_getMethodImplementation([self class], @selector(ZKKeyboardpan:)), method_getTypeEncoding(class_getInstanceMethod([self class], @selector(ZKKeyboardpan:))));
...
- (void)keyboardDisappear:(NSNotification *)note
{
    ...
    [UIView animateWithDuration:duration
                     animations:^{
                          ...
                          *比如那个文件中有用到transform，结果这里给它回到了Identity 就血崩了
                          */
...
- (void)setTextFieldInputAccessoryView
{
    ...
    if (!tempView) {
        ...
        //滑动用来控制光标的view
        static CGFloat width4SlideView = 200;
        static CGFloat height4SlideView = 20;
        UIView *aSlideView = [[UIView alloc]initWithFrame:CGRectMake((tempView.frame.size.width - width4SlideView)*0.5,
                                                                       (tempView.frame.size.height-height4SlideView)*0.5,
                                                                       width4SlideView,
                                                                       height4SlideView)];
        aSlideView.backgroundColor = [UIColor lightGrayColor];
        aSlideView.layer.cornerRadius = 8;
        [tempView addSubview:aSlideView];
        
        UIPanGestureRecognizer *aPanGesture = [[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(ZKKeyboardpan:)];
        [aSlideView addGestureRecognizer:aPanGesture];
        for (id i in self.view.subviews)
        {
            if ([i isFirstResponder]) {
                firstResponder = i;
            }
        }
    }
...
//滑动控制光标的响应
static CGFloat recorderX = 0.0;
- (void)ZKKeyboardpan:(UIPanGestureRecognizer *)aPanGesture
{
    if (aPanGesture.state == UIGestureRecognizerStateChanged) {
        CGPoint gesturePoint = [aPanGesture locationInView:firstResponder];
        CGPoint gestureVelocity = [aPanGesture velocityInView:firstResponder];
        NSLog(@"point:%@,velocity:%@",NSStringFromCGPoint(gesturePoint),NSStringFromCGPoint(gestureVelocity));
    }
